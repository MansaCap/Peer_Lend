...
def signup(request: AuthRequest):
    user_handle = request.email.split("@")[0]
    return AuthResponse(token=f"new-user-token-{user_handle}", user_id=2)
...
def integration_hooks(request: IntegrationHookRequest):
    ...
    return IntegrationHookResponse(
        success=True,
        message=f"Processed {request.provider} payload",
    )
